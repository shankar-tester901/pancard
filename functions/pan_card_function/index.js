...
app.post('/upload', async function(req, res) {
    ...
    let panDetails = await getPanDetails(uploadPath, catalystApp);
...
function getPanDetails(path, catalystApp) {
    return new Promise((resolve, reject) => {
        console.log('in getDetails2   ');

        var zia = catalystApp.zia();
        zia.extractOpticalCharacters(fs.createReadStream(path), { modelType: 'PAN' })
            .then((result) => {
                //    console.log(result);
                //   return result;
                resolve(result);
